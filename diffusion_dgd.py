...
from UNet import GraphUNet, HalfHalfGraphUNet
...
class DiffusionUNet(torch.nn.Module):
    def __init__(self, nx_graph_list, cfg):
        ...
        if cfg["model"] == "half-half":
            self.model = HalfHalfGraphUNet(in_channels=self.x_dim + 1 + self.features_dim,
                                   # Currently passing t as a node-level feature
                                   hidden_channels=hidden_dim,
                                   out_channels=self.x_dim,
                                   num_layers=int(cfg["n_layers"]),
                                   depth=int(cfg["depth"]),
...
def main(cfg : DictConfig) -> None:
    ...
    elif cfg["name"] == "community":
        setup_wandb(cfg)
        graphs = get_community_dataset(int(cfg["n_samples"]))
        print(graphs)
